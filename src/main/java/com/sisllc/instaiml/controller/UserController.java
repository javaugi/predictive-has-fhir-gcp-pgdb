...
public class UserController {
    ...
    public Flux<User> getAllUsers() {
        ...
        return userService.getAllUsers().limitRate(20)
            ...
            .switchIfEmpty(Mono.empty()
                .doOnNext(dummy -> log.info("Empty getAllUsers {}", dummy))
                .then(Mono.empty())
            )
            .doOnError(e -> log.error("Error getAllUsers {}", e.getMessage()));
    }
    ...
    public Mono<ResponseEntity<Void>> delUserById(@PathVariable("id") String id) {
        log.info("deleteUserById id {}", id);
        return userService.deleteUserById(id)
            .then(Mono.just(ResponseEntity.noContent().build()))
            .onErrorResume(UserNotFoundException.class, ex
